...
;|                                                                      |
;|                                                                      |
;| code by Moon/ABYSS                                     November 1995 |
;------------------------------------------------------------------------
determblink=0
usesection=1
...
;----------
;----------
angsincorr:	;-->a0, base of sinus
	move.w	#(angsinusend-angsinus)/2-1,d7
angsincorrloop:
	move.w	(a0),d0
	cmp.w	#360,d0
angsincheck:
	...
	bra.b	angsincheck
angsinok:
	move.w	d0,angsinusend-angsinus(a0)
	move.w	d0,(a0)+
	dbf	d7,angsincorrloop
	rts
;----------
;----------
coppercopy:
...
fadetextl1:
	move.l	(a2),d0			;pos
	bmi.b	fadetext_skipbob	;bob not active
	move.w	4(a2),d1		;animpoint
	cmp.w	#15*2*24,d1		;last animpic?
	bne.b	fadetext_notlastanimpic	;no signore
...
;	mulu	#2*24,d1		;animpicnumber to animpicoffset
	...
	bra.b	fadetext_skipinsert

fadetext_skipbob:
	tst.w	fadetextnewbobs		;counter how many bobs to add...
	beq.b	fadetext_skipinsert	;...this frame
...
fadetext_skipinsert:
	...
	bne.b	fadetext_skip		;yes-->back to the roots
...
askkey:
	...
	beq.w	askkey_skip		;not full, irq not from keyboard!
...
organize_aon:
	move.l	(a7)+,a1
	move.l	(a1),a1	;mem of patterns
	lea	mod_smplX,a0	;mem for aon-patterns
	jsr	determ		;decrunch

	move.w	#%1010000000000000,intena(a5)
	lea	mod_smplX,a0	;mem for aon-patterns
	lea	aon_code,a6
	moveq	#0,d0
	jsr	8(a6)
...
;------------
charsize=2*11
texthashtable:	dcb.w	256,0
textcalc:
	lea	charsend,a2
	lea	texthashtable,a4
	move.l	a4,-(a7)
	move.w	#255,d7
	moveq	#0,d0
textcalc_calchashl1:
...
textcalc_calchashl2:
	cmp.b	-(a3),d0	
	beq.b	textcalc_calchashl3
	dbf	d6,textcalc_calchashl2
	moveq	#-2,d6
...
titlecol:;destination
	dc.w $323,$FFD,$EEB,$DCA,$CB8,$BA7,$A86,$974
	dc.w $863,$743,$632,$521,$411,$310,$200,$100
	dc.w $EEE,$CDD,$BCC,$ABB,$9AA,$899,$688,$587
	dc.w $577,$466,$355,$244,$233,$122,$223,$201
...
titlepic:	incbin	"data/titlepic8.blt"
dcb.b	40*5*16,0
...
mod_smpl1:
	incbin	"data/modules/SMP.1_terraforming.term"
even
mod_smpl2:
	dc.w	-1
even
mod_smpl3:
	incbin	"data/modules/SMP.3_eclipse.term"
even
mod_smpl4:
	dc.w	-1
even
mod_smpl5:
	incbin	"data/modules/SMP.5_afternoon_voyage_ii.term"
even
mod_smpl6:
	dc.w	-1
even
mod_smpl7:
	incbin	"data/modules/SMP.7_alleyway_chat.term"
even
mod_smpl8:
	dc.w	-1
even
...
mod_data1:                          
	incbin	"data/modules/p60.1_terraforming"
even
mod_data2:                          
	incbin	"data/modules/aon.2_Inner_voyage.term"
even
mod_data3:                          
	incbin	"data/modules/p60.3_eclipse"
even
mod_data4:                          
	incbin	"data/modules/aon.4_cradle_of_the_futur.term"
even
mod_data5:                          
	incbin	"data/modules/p60.5_afternoon_voyage_ii"
even
mod_data6:                          
	incbin	"data/modules/aon.6_Fo-o-llin_around.term"
even
mod_data7:                          
	incbin	"data/modules/p60.7_alleyway_chat"
even
mod_data8:                          
	incbin	"data/modules/aon.8_Exoneration.term"
even
mod_data9:                          
	incbin	"data/modules/aon.9_Steel_Breeze.term"
even
mod_data10:                         
	incbin	"data/modules/aon.a_trough_the_riots.term"
even
...
aon_code:	incbin	"data/aonreplay.bin"
even
...
spiral:
	...
	lea	angsinuspoint(pc),a3
	move.w	(a3),d0
	addq.w	#2,d0
	cmp.w	#angsinusend-angsinus,d0
	bne.b	noangsinusflow
	moveq	#0,d0
noangsinusflow:
	move.w	d0,(a3)+
	add.w	d0,a3

	move.w	startangle2(pc),d5	;angle
	addq.w	#2,d5			;spiral turn speed
	move.w	(angsinusend-angsinus)*2(a3),d5
	bpl.b	nonegrad2
	add.w	#360,d5
nonegrad2:
	cmp.w	#360,d5
...
noflowrad2:
	...
	move.w	d5,startangle2
	swap	d5
	move.w	startangle1(pc),d5	;angle
	addq.w	#2,d5			;spiral turn speed
;	move.w	(a3),d5
	bpl.b	nonegrad1
	add.w	#360,d5
nonegrad1:
	...
	sub.w	#360,d5
noflowrad1:
	move.w	d5,startangle1


	lea	sinus,a1
;	lea	koordstore(pc),a4

	lea	radsinuspoint(pc),a6
	move.w	(a6),d0
	addq.w	#2,d0
	cmp.w	#radsinusend-radsinus,d0
	bne.b	noradsinusflow
	moveq	#0,d0
noradsinusflow:
	move.w	d0,(a6)+
	add.w	d0,a6

	moveq	#startrad2,d6		;radius
	add.w	(radsinusend-radsinus)*2(a6),d6
	swap	d6
	move.w	#startrad1,d6		;radius
	add.w	(a6),d6
...
spirall1:
	...
	moveq	#0,d2
;	moveq	#0,d3
	move.w	lastpointx(pc),d2
	move.w	lastpointy(pc),d3

	move.w	d0,lastpointx
	move.w	d1,lastpointy
...
;	moveq	#0,d1
	moveq	#0,d2
;	moveq	#0,d3
	move.w	lastpointx(pc),d0
	move.w	lastpointy(pc),d1
	move.w	firstpointx(pc),d2
	move.w	firstpointy(pc),d3
	bsr.w	line
...
firstline:
	move.w	d0,firstpointx
	move.w	d1,firstpointy
	bra.w	afterfirstline
...
radsinuspoint:	dc.w	0
radsinus:
incbin	"data/radsinus1.bin"
radsinusend:
dcb.b	radsinusend-radsinus,0

incbin	"data/radsinus2.bin"
dcb.b	radsinusend-radsinus,0


angsinuspoint:	dc.w	0
angsinus:	incbin	"data/angsinus1.bin"
angsinusend:	dcb.b	angsinusend-angsinus,0

angsinus2:	incbin	"data/angsinus2.bin"
		dcb.b	angsinusend-angsinus,0
